...
function playnow(vsrc){
	...
	audio.src='http://patchamama.com/_audios/'+vsrc;
    audio.play();
...
<a href="?filter=mp3-youtube">Selección mp3</a> |
	...
	if 	($vfilter<>".*") {
		...
		foreach ($dirs as $id) {
			...
			foreach ($vfiledir as $id) {
				...
				if ( ( ($vfilter==tostr("youtube").".*") or ($vfilter==tostr("mp3-youtube").".*") )  and (stripos("  ".$id,"mp3-youtube")>0) )  {
					$vfollow = True;
					}
				...
				if ( (preg_match('/'.$vfilter.'/i', $b1)) or ($vfollow) ) {
					...
					if ((stripos($b1,".epub")>0) || (stripos($b1,".mp3")>0) || (stripos($b1,".mp4")>0) ) {
						...
						if ($vlastdir==$a) {
							...
							echo "\r\n".'.......'.$vimag.'<a href="#'.($fileref).'" mp3ref="'.($fileref).'" id="myLink" onclick="playnow(\''.($fileref).'\');return false;" >'.($b).'</a>'.($vyoutube).'<br />';
							}else{
